...
const convertProperty = (jsonSchema: InputJsonSchema): InputSchemaProperty => {
  ...
  switch (jsonSchema.type) {
    ...
    case 'array':
      property.type = 'array';
      if (jsonSchema.items) property.items = convertProperty(jsonSchema.items);
      break;
    case 'object':
  ...
  if (jsonSchema.multiline === true) property.multiline = true;
  if (typeof jsonSchema.label === 'string') property.label = jsonSchema.label;
